...
{
    internal class EFTimerProcessSelectQuery<TId, TDbContext, TEntity> 
        : IProcessSelectQuery<TId>
        where TDbContext : DbContext
        where TEntity : TimerProcessDbEntity<TId>
    {
        ...
        private readonly TimerProcess_AsyncExecute_Condition<TId, TEntity> _timerProcess_AsyncExecute_Condition;
                ...
                await using (var transaction = await _transactionManager.StartTransactionAsync(cancellationToken))
                {
                    using (var hint = _lockQueryHintStore.StartScope(LockHintEnum.ForNoKeyUpdateAndSkipLocked))
                    {
                        ...
                        var batch = await _dbContext.Set<TEntity>()
                            ...
                            .ApplayFilterCondition(_timerProcess_AsyncExecute_Condition, now)
                        ...
                        result = batch
                            .Select(e => new ProcessInstanceInfoDto<TId>(
